...
sibylapp could always use more documentation, whether as part of the
official sibylapp docs, in docstrings, or even on the web in blog posts,
articles, and such.
...
Ready to contribute? Here's how to set up `sibylapp` for local development.

1. Fork the `sibylapp` repo on GitHub.
2. Clone your fork locally::

    $ git clone git@github.com:your_name_here/sibylapp.git

3. Install your local copy into a virtualenv. Assuming you have virtualenvwrapper installed,
   this is how you set up your fork for local development::

    $ mkvirtualenv sibylapp
    $ cd sibylapp/
...
2. The tests that cover a module called ``sibylapp/path/to/a_module.py``
   should be implemented in a separated module called
   ``tests/sibylapp/path/to/test_a_module.py``.
...
To run a subset of tests::
    ...
    $ python -m pytest tests.test_sibylapp
...
2. Update the version in ``setup.cfg``, ``sibylapp/__init__.py`` and
   ``HISTORY.md`` files.
3. Create a new git tag pointing at the corresponding commit in ``stable`` branch.
4. Merge the new commit from ``stable`` into ``master``.
5. Update the version in ``setup.cfg`` and ``sibylapp/__init__.py``
...
or in command line::
    ...
    pip install 'sibylapp>=X.Y.Z.dev'


.. _GitHub issues page: https://github.com/HDI-Project/sibylapp/issues
.. _Build Status page: https://travis-ci.org/HDI-Project/sibylapp/pull_requests
